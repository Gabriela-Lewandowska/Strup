...
def run_trial(win, conf, stim, fix_cross, clock,trial_no,podpowiedz):
    ...
    if trial_no in zgodnosc or trial_no in trening:
        ...
        if kolor=="pink":
            stim.text, stim.color = "RÓ\u017BOWY","pink"
        if kolor=="red":
            stim.text, stim.color = "CZERWONY", "red"
        if kolor=="green":
            stim.text, stim.color = "ZIELONY", "green"
        if kolor=="blue":
            stim.text, stim.color = "NIEBIESKI", "blue"
    else:
        kolor = random.choice(["pink", "red", "green", "blue"])
        bodziec=0
        if kolor=="pink":
            stim.color="pink"
            stim.text=random.choice(["CZERWONY", "ZIELONY", "NIEBIESKI"])
        if kolor=="red":
            stim.color="red"
            stim.text=random.choice(["RÓ\u017BOWY", "ZIELONY", "NIEBIESKI"])
        if kolor=="green":
            stim.color="green"
            stim.text=random.choice(["RÓ\u017BOWY","CZERWONY", "NIEBIESKI"])
        if kolor=="blue":
            stim.color="blue"
            stim.text=random.choice(["RÓ\u017BOWY","CZERWONY", "ZIELONY"])
