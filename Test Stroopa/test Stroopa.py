...
def main():
    ...
    for csi in range(15):
        keyPressed, rt, kolor, bodziec, przycisk=run_trial(win, conf, stim, fix_cross, clock,trial_no,podpowiedz)
        if keyPressed==przycisk:
            corr=1
        else:
            corr=0
...
def run_trial(win, conf, stim, fix_cross, clock,trial_no,podpowiedz):
    ...
    if trial_no in zgodnosc or trial_no in trening:
        bodziec=1
        kolor, stim.text, przycisk = random.choice([("pink", "ROZOWY", "1"), ("red", "CZERWONY", "2"), ("green", "ZIELONY", "9"), ("blue", "NIEBIESKI", "0")])
        stim.color=kolor

    else:
        bodziec = 0
        kolor = random.choice(conf['COLORS'])
        tablicaSlow = conf['STIM_TEXT']
        stim.color = kolor
        if kolor == "pink":
            nowa = tablicaSlow.remove("ROZOWY")
            stim.text = random.choice(tablicaSlow)
            tablicaSlow.append("ROZOWY")
            przycisk="1"
        if kolor == "red":
    ...
    return keyPressed, rt, kolor, bodziec, przycisk
